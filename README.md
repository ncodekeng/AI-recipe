...
After Azure citation verification, PLATE attempts to fetch only that exact publisher URL and parse its `schema.org/Recipe` JSON-LD or legacy Recipe microdata. Successful extraction verifies the publisher title and replaces model-structured ingredient text, times, servings, and calories with values read directly from the page. Deterministic dietary and allergen validation then runs again over the enriched ingredient list. The outbound client disables redirects, accepts only ordinary public HTTPS destinations, blocks local/private addresses at connection time, limits content type, response size, concurrency, and timeout, and treats page content as untrusted. If a publisher blocks access or provides no readable Recipe structured data, PLATE keeps the already citation-verified Azure result instead of failing or fabricating fields. Extraction does not republish `recipeInstructions`; without a content licence, the clearly labelled AI guide remains separate and the publisher link remains canonical.

Publisher extraction is enabled by default. Configure it with `RecipeCatalog__PublisherExtraction__Enabled`, `MaxResponseBytes`, `MaxConcurrentRequests`, `CacheEnabled`, `CacheDurationHours`, and `CacheMaxEntries`. Parsed page metadata can be cached by exact source URL for up to 24 hours, but the same `RecipeCatalog__Cache__ProviderPermissionConfirmed=true` gate is required; with the default production settings it is not stored. Extraction does not use Gemini or consume another AI request; Gemini URL Context remains an optional future fallback for pages whose structured data cannot be read.
...
For visual testing, the tracked `dotnet run` development profile sets `RecipeCatalog__CommercialImages__AllowUnverifiedForTesting=true`. When no fully verified Commons photo is found, Development prefers the exact image declared by verified publisher Recipe JSON-LD or microdata, then may use a relevant Commons web image. Both display an orange **Unverified · testing only** warning. The backend requires both that flag and the Development host environment, so these fallbacks remain disabled in production even if the normal production example settings are used. Recipe structured data proves image provenance, not commercial reuse rights.
...
The separate recipe cache can save repeated Azure web-search or Edamam calls. It uses a safety-aware key containing the active provider, active prompt revision, normalized ingredient names, Main ingredient, allergens, avoided ingredients, recent result IDs, diet, cooking time, servings, requested result count, recipe mode, photo preference, and publisher-extraction mode. It stores the ranked recipe data before photos are attached. The independent photo cache stores the complete verified image/license object (or a negative lookup) by normalized dish and whitelist configuration for up to seven days. Publisher structured-data extraction has its own short cache keyed by exact cited source URL. All caches are server-memory caches, so a process restart or Azure App Service recycle clears them.
...
$env:RecipeCatalog__PublisherExtraction__Enabled = 'true'
$env:RecipeCatalog__PublisherExtraction__CacheEnabled = 'true'
$env:RecipeCatalog__PublisherExtraction__CacheDurationHours = '24'
$env:RecipeCatalog__PublisherExtraction__CacheMaxEntries = '500'
```
...
The backend tests cover forced Azure web search, citation enforcement, strict HTTPS sourcing, Main ingredient enforcement, 40/60 match scoring, halal wine suppression, ingredient aliases, missing-ingredient calculation, pantry basics, exact/near-match ranking, bounded publisher Recipe JSON-LD and legacy microdata extraction, SSRF address rejection, publisher-page caching, commercial-image license and attribution rejection, separate photo-cache metadata, image serialization, and the Deliveroo handoff. Frontend tests cover Main ingredient/result-count request fields, match explanations, recipe-specific artwork, commercial-license metadata, testing-only publisher images, attribution, photo ON/OFF/failure decisions, preference persistence, missing-only basket payloads, and the zero-missing case. The credential-free smoke suite checks image upload, refusal to fabricate recipes, usage tracking, quota enforcement, grocery handoff, and feedback rate limiting. Real Azure, publisher pages, Wikimedia Commons, and optional Edamam responses still require staging verification.
...
- `POST /api/recipes/photos` — finds verified reusable Commons photos for up to six existing recipe results without another AI recipe request; local Development may fall back to an explicitly labelled publisher image from verified Recipe JSON-LD or microdata
