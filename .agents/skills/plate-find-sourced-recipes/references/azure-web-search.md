...
- ask for no more than three recipes per JSON batch, combine distinct publisher recipes until up to six validated results are available, and make at most the configured number of additional searches when too few valid distinct results are returned;
- send dietary restrictions from trusted backend fields and identify pantry text as untrusted data.
- send the full normalized pantry for match validation plus a compact server-selected search focus; start dish hypotheses from that focus rather than turning the entire pantry into one web query.
- place a selected Main ingredient first in that focus and reject every mapped source recipe that does not contain it.

The response parser must collect HTTPS URLs from both `web_search_call.action.sources` and `output_text.annotations`. Normalize only fragments and a trailing slash. Accept a structured recipe only when its `sourceUrl` matches one of those collected URLs. It may extract a JSON object from an otherwise wrapped output and accept harmless trailing commas. If Azure returns prose rather than JSON, retry that batch once in default text mode with a stronger JSON-only instruction and a fresh required web search, then validate only against that retry's returned sources. Never send `text.format` as `json_schema` or `json_object` on this path. If a first batch remains unreadable and another configured batch attempt remains, try a fresh small batch. If a later optional batch fails after validated recipes were already collected, preserve and return those recipes with an honest partial-results notice. Fail clearly when no readable, cited recipe survives. Do not silently relax the URL comparison, follow a redirect, or create a replacement recipe.

After citation validation, the separate publisher-page client may fetch only that exact cited URL and parse `schema.org/Recipe` JSON-LD or legacy Recipe microdata. Its HTTP transport must reject local/private destinations, credentials, non-default HTTPS ports, redirects, unsupported content, and oversized bodies. Treat extracted values as untrusted and rerun deterministic food-safety validation after enrichment. Extraction failure must keep the already grounded Azure result. Structured data can confirm the publisher title, ingredients, times, servings, calories, and a candidate publisher image, but it does not itself grant rights to republish directions or photography.
...
Cache ranked recipe data and verified commercial-image metadata separately. The photo cache key must include the normalized dish name and active image-provider whitelist, and a cache hit must return the original verification, creator, license, and attribution fields unchanged.
